...
class OrganizationServiceTest {
    ...
    void join() {
        ...
        List<Organization> memberOrganizations = organizationRepository.findByMemberOrganizations(lala);
        assertAll(
                () -> assertThat(memberOrganizations).hasSize(1),
                () -> assertThat(memberOrganizations.get(0).getOrganizationMembers().getValues().get(0).getOrderNumber()).isEqualTo(1),
                () -> assertThat(memberOrganizations.get(0).getOrganizationMembers().getValues().get(0).isLastAccessed()).isTrue()
        );
    }
}
