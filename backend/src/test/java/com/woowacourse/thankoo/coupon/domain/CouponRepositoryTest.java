...
class CouponRepositoryTest {
    ...
    void updateCouponStatus() {
        ...
        List<Long> couponsIds = IntStream.range(0, 3)
                .mapToObj(coupon -> couponRepository.save(givenCoupon(sender, receiver, organization.getId())))
                .map(Coupon::getId)
    ...
    private Coupon givenCoupon(final Member sender, final Member receiver, final Long organizationId) {
        return new Coupon(organizationId, sender.getId(), receiver.getId(), new CouponContent(TYPE, TITLE, MESSAGE),
                NOT_USED);
    }
}
